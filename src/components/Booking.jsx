...
export default function Booking() {
  ...
  // Measure hero block height (and track mobile breakpoint) so the form
  // section knows exactly how far to slide up and cover it.
  useEffect(() => {
  ...
  // Reset the lifted state whenever we land back on step 1 (fresh view).
  useEffect(() => {
    if (formStep === 1) setFormLifted(false);
  }, [formStep]);

  // On mobile step 1, scrolling slides the form up over the hero text.
  useEffect(() => {
    if (formStep !== 1 || !isMobile) return;
    const handleScroll = () => {
      setFormLifted(window.scrollY > 8);
    };
      ...
      >
        ...
        {formStep === 1 && (
          <section className="flex flex-col items-center xl:items-start text-center xl:text-left px-6 md:px-12 pt-6 xl:pt-16 xl:sticky xl:top-16">
            <div
              className={`w-full overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] ${isMobile && formLifted ? 'max-h-0 opacity-0' : 'max-h-[40rem] opacity-100'
                }`}
            >
              {/* Breadcrumb */}
              <Link
              ...
              <div className="booking-hero-text opacity-0 flex items-center justify-center lg:justify-start gap-3 mt-4 lg:pl-4">
                <a href="mailto:juneco.mirande@gmail.com?subject=Let%27s%20work%20together" data-cursor-text="Email" data-cursor-style="tooltip" className="w-9 h-9 rounded-full border border-primary/20 flex items-center justify-center text-primary/50 hover:text-accent hover:border-accent transition-colors duration-200">
                  <EnvelopeSimple size={16} />
                </a>
                <a href="https://www.linkedin.com/in/juneco-mirande/" target="_blank" rel="noopener noreferrer" data-cursor-text="View" data-cursor-style="tooltip" className="w-9 h-9 rounded-full border border-primary/20 flex items-center justify-center text-primary/50 hover:text-accent hover:border-accent transition-colors duration-200">
        ...
        <section
          className="w-full overflow-visible mx-auto flex flex-col gap-4 relative z-20"
          onFocusCapture={() => formStep === 1 && isMobile && setFormLifted(true)}
          onClickCapture={() => formStep === 1 && isMobile && setFormLifted(true)}
        >
