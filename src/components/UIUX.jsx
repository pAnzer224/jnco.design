...
export default function UIUX({ setActivePage }) {
  ...
  useEffect(() => {
    if (openModal !== null) {
      ...
      if (works[openModal].type !== "scrollable-image") {
  ...
  return (
    ...
    >
      ...
      {openModal !== null && (
        ...
        >
          {(works[openModal].type === "link" || works[openModal].type === "figma") && (
            <a
              href={works[openModal].type === "link" ? works[openModal].url : works[openModal].src}
              target="_blank"
            ...
            >
              <span className="font-mono text-xs uppercase tracking-widest font-bold">{works[openModal].type === "figma" ? "Open in Figma" : "Visit Website"}</span>
